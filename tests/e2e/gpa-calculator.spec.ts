...
test.describe('GPA Calculator', () => {
    ...
    test('should display GPA calculator widget', async ({ page }) => {
        // Check heading
        await expect(page.locator('.gpa-calculator h3')).toContainText('GPA Calculator');
        ...
        await expect(page.getByRole('button', { name: /calculate gpa/i })).toBeVisible();
    });

    test('should calculate semester GPA correctly', async ({ page }) => {
        // Fill first course: Course 1, Grade A (4.00), 3 credits
        await page.locator('.course-row').first().locator('.course-name').fill('Data Structures');
        await page.locator('.course-row').first().locator('.grade').selectOption('4.00');
        await page.locator('.course-row').first().locator('.credits').selectOption('3');

        // Add second course
        await page.getByRole('button', { name: /add course/i }).click();

        // Fill second course: Course 2, Grade B (3.00), 3 credits
        await page.locator('.course-row').nth(1).locator('.course-name').fill('Web Development');
        await page.locator('.course-row').nth(1).locator('.grade').selectOption('3.00');
        await page.locator('.course-row').nth(1).locator('.credits').selectOption('3');
    ...
    test('should show validation error for incomplete course', async ({ page }) => {
        // Fill name but not grade/credits
        await page.locator('.course-row').first().locator('.course-name').fill('Incomplete Course');
        ...
        await expect(page.locator('#validation-error')).toContainText(/grade and credits/i);
    });
    ...
    test('should calculate new CGPA with previous data', async ({ page }) => {
        ...
        await page.locator('.course-row').first().locator('.course-name').fill('Final Year Project');
        await page.locator('.course-row').first().locator('.grade').selectOption('4.00');
        await page.locator('.course-row').first().locator('.credits').selectOption('3');
        ...
        expect(parseFloat(newCgpaText || '0')).toBeGreaterThan(3.20);
        expect(parseFloat(newCgpaText || '0')).toBeLessThanOrEqual(4.00);
    });
