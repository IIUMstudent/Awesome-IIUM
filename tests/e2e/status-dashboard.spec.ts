...
test.describe('Status Dashboard', () => {
    ...
    test('should load and display system statuses', async ({ page }) => {
        ...
        await expect(page.locator('.status-card')).toHaveCount(await page.locator('.status-card').count(), { timeout: 10000 });
        ...
        for (let i = 0; i < Math.min(cardCount, 3); i++) {
            await expect(page.locator('.status-card').nth(i).locator('.status-header')).toBeVisible();
            await expect(page.locator('.status-card').nth(i).locator('.status-body')).toBeVisible();
        }
    });
    ...
    test('should display status indicators correctly', async ({ page }) => {
        ...
        if (count > 0) {
            ...
            const statusText = await cards.first().locator('.status-label').textContent();
    ...
    test('should display last checked timestamp', async ({ page }) => {
        ...
        if (count > 0) {
            ...
            const timeText = await cards.first().locator('.status-time').textContent();
